...
- **`doas-user-nopass.conf` + `sudoers-user-nopass.conf`** give the default `user` passwordless root. Mirrors the live install fix so it survives fresh flashes. The device package depends on `doas` (not `doas-sudo-shim`) — current pmaports `postmarketos-base-systemd` ships `sudo-rs` which provides real `sudo`, and `doas-sudo-shim` apk-conflicts with it. Both `doas` and `sudo` work after install.
...
- **`smb_get_iio_chan()` returns 0 for `VOLTAGE_NOW`/`CURRENT_NOW` when `STATUS != Charging`.** This is by design upstream (see `drivers/power/supply/qcom_smbx.c:560`). When debugging, don't chase non-zero readings on a battery at 100 % — drain to ≤ 95 % and replug; the ADC channels (`usb_in_i_uv`, `usb_in_v_div_16`) read fine when the driver enters the Charging branch.
...
- **`phoenix-charge-cap.timer` exists for 24/7 server use; disabled by default.** Hysteresis daemon (60-70% by default) that pauses charging by writing `Unknown` to `/sys/class/power_supply/pm8150b-charger/status` — which the `qcom_smbx` driver maps to `USBIN_SUSPEND_BIT` in `USBIN_CMD_IL`. USB data path stays up (verified). Opt in with `systemctl enable --now phoenix-charge-cap.timer`. Adjust thresholds in `/etc/phoenix-charge-cap.conf` (`STOP=`, `START=`).
...
- `--firmware-root <path>` — recommended; packages a full `lib/firmware/` tree and verifies all 8 required blobs are present (`a615_zap.mbn`, `adsp.mbn`, `cdsp.mbn`, `modem.mbn`, `wlanmdsp.mbn`, `ipa_fws.mbn`, `venus.mbn`, `novatek_nt36672c_g7b_fw01.bin`). Strips duplicates already provided by `firmware-qcom-adreno-a630` (`a630_gmu.bin`, `a630_sqe.fw`) to avoid APK file-conflicts.
