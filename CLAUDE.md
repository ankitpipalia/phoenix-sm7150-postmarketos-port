...
- **Patch 0010 keeps SMB input telemetry available while charge behavior is inhibited.** It also corrects SMB5 status/OV/ICL decoding and prescaled voltage handling. **Patch 0011 adds watchdog base, OV recovery, float off-by-one, ICL downgrade ordering, log throttling and revalidation robustness per the 5-patch Fixes + P0/P1 review.** Validate the readings against QGauge/TCPM on hardware after every kernel rebase.
