...
| `phoenix-llm-manager.{py,service,conf,html,css,js}` | Single-runtime llama.cpp manager, chat test, logs, memory/thermal monitor, and detailed battery/charger/TCPM dashboard at port 7070. Includes safe Q8 and experimental true Turbo4 KV-cache profiles for Spark-X2.5. | Enabled by default; requires the custom runtime at the configured `LLAMA_SERVER` path before a model can start. | `/etc/phoenix-llm-manager.conf`; set `API_TOKEN` on untrusted networks |
